...
def test_normalized_ap_with_different_prevalences():
    ...
    meta1 = pd.DataFrame({
        'treatment': ['compound_X'] * 5 + ['DMSO'] * 95,
        'id': range(100)
    })
    ...
    meta2 = pd.DataFrame({
        'treatment': ['compound_Y'] * 50 + ['DMSO'] * 50,
        'id': range(100)
    })
    ...
    norm1 = result1[result1['treatment'] == 'compound_X']['normalized_average_precision'].mean()
    norm2 = result2[result2['treatment'] == 'compound_Y']['normalized_average_precision'].mean()
    ...
    assert norm_diff <= raw_diff, "Normalization should not increase prevalence-based differences"
...
def test_mean_average_precision_with_normalization():
    ...
    for treatment in ['A', 'B', 'C']:
        ...
        actual_norm_map = result.loc[result['treatment'] == treatment, 
                                     'mean_normalized_average_precision'].values[0]
